@php
    ...
    $styleClasses = match($style ?? 'cards') {
        ...
        'minimal' => 'p-4',
        default => 'bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6',
    };
...
<section
    class="team-block {{ $sectionSpacing }} pb-16 sm:pb-24"
        ...
        @if(!empty($members))
            <div class="grid gap-8 {{ $columnsClass }}">
                @foreach($members as $member)
                    <div class="team-member {{ $styleClasses }} {{ $textAlignClass }}">
                        {{-- Photo --}}
                        <div class="mb-4 {{ $textAlignClass === 'text-center' ? 'flex justify-center' : '' }}">
                            @if(!empty($member['photo']))
                                <img
                                    ...
                                    alt="{{ $member['name'] ?? '' }}"
                                    class="w-24 h-24 sm:w-32 sm:h-32 object-cover {{ $imageClasses }}"
                                >
                            @else
                                <div class="w-24 h-24 sm:w-32 sm:h-32 {{ $imageClasses }} bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
                                    <span class="text-3xl sm:text-4xl font-semibold text-primary-600 dark:text-primary-400">
                        ...
                        {{-- Name & Role --}}
                        <h3 class="team-member-name font-semibold text-lg" style="color: var(--block-heading-color);">
                            {{ $member['name'] }}
                        </h3>
                        <p class="team-member-role text-sm mt-1" style="color: var(--block-text-color);">
                            {{ $member['role'] }}
                        </p>

                        {{-- Bio --}}
                        @if(($show_bio ?? true) && !empty($member['bio']))
                            <p class="team-member-bio text-sm mt-3 leading-relaxed" style="color: var(--block-text-color);">
                                {{ $member['bio'] }}
                            </p>
                        @endif
