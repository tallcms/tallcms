        ...
        @foreach($posts as $post)
            <li class="flex gap-3">
                @if($showImage && $post->featured_image)
                    <a href="{{ url(tallcms_localized_url($post->slug)) }}" class="flex-shrink-0">
                        <img
                ...
                @endif
                <div class="min-w-0">
                    <a href="{{ url(tallcms_localized_url($post->slug)) }}" class="link link-hover font-medium text-sm line-clamp-2">
                        {{ $post->title }}
                    </a>
