            ...
            @if(($layout ?? 'list') === 'cards')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($documents as $doc)
                        @php $config = $getFileConfig($doc->mime_type); @endphp
                        <a
                            href="{{ $doc->download_url }}"
                            download
                            class="block p-4 rounded-lg border border-base-300 hover:border-primary hover:shadow-md transition-all group"
                        >
