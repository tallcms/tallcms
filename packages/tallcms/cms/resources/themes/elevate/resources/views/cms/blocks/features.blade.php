            ...
            <div class="grid gap-6 sm:gap-8 {{ $columnsClass }}">
                @foreach($features as $index => $feature)
                    @php
                        ...
                        // Elevate: first card gets subtle emphasis via tint + border
                    ...
                    >
                        @if($hasLink)
                            <a href="{{ e($feature['link']) }}"
                               class="{{ $elevateCardClass }} {{ $text_alignment ?? 'text-center' }} cursor-pointer {{ $isIconLeft ? 'card-side' : '' }} h-full">
                        @else
                            <div class="{{ $elevateCardClass }} {{ $text_alignment ?? 'text-center' }} {{ $isIconLeft ? 'card-side' : '' }} h-full">
                        @endif

                            <div class="card-body {{ $isIconLeft ? 'flex-row items-start gap-4' : '' }}">
                                {{-- Icon --}}
                                <div class="{{ $isIconLeft ? 'flex-shrink-0' : 'flex justify-center mb-4' }}">
                                    <div class="{{ $iconContainerClass }} rounded-xl bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center">
                                        @if($isValidHeroicon)
                                            <x-dynamic-component
                                                :component="$iconName"
